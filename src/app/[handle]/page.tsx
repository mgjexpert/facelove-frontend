...
export default async function AnaPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const { assets, posts, packs, live, mode } = await getSpaceContent(access);
  ...
  return (
    <main className="profile-page">
      ...
      <div className="profile-shell">
        ...
        <ProfileTabs posts={posts} assets={visibleAssets} packs={packs} access={access} live={live} mode={mode} bio={ana.profile.bio} />
      </div>
    </main>
  );
}
