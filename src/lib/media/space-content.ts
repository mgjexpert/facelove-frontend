...
export async function getPersistedContent(username: string, albums: SpaceAlbum[], editorial: SpacePost[], media: PublicMedia[], postMedia: PostMedia[]) {
  const catalog = await getGatewayCatalog(username);
  const permitted = new Map(await Promise.all(albums.map(async album => [album.id, await hasAlbumAccess(album.id)] as const)));
  ...
  const assets: MediaAsset[] = [...publicAssets, ...(catalog?.assets ?? []).filter(asset => permitted.get(asset.packId || ""))];
  ...
  const packs: VisiblePack[] = albums.map(album => ({ id: album.id, title: album.title,
    ...
    locked: !permitted.get(album.id),
  }));
  for (const album of albums) {
    const unlocked = permitted.get(album.id);
  ...
  return { assets, posts, packs, live: Boolean(catalog || publicAssets.length), mode: catalog ? "gateway" as const : "unavailable" as const,
    access: [...permitted.values()].some(Boolean) };
}
